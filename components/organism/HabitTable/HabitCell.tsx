...
const HabitCell = ({ measurable, value, onClick, color, isCompleted, notes, disabled}: HabitCellProps) => {

  
  const colorClass = value && isCompleted ?  `text-${color}-500` : "text-gray-500";

  return (
    <td className={`text-center relative cursor-pointer ${disabled ? "opacity-25 cursor-not-allowed":""}`} 
     onClick={disabled ? undefined : onClick} data-index={disabled}>
